...
# x <- dd$lat
# y <- 	data.frame((1/r_nifcell_h)*(dd$richelia+1),
#                  (1/ucynb_nifcell_h)*(dd$ucyn_b+1),
#                  (1/ucyna_nifcell_h)*(dd$ucyn_a+1),
#                  (1/t_nifcell_h)*(dd$trichodesmium+1))
# 
# areaplot(x,y)
...
par(mfrow=c(2,2),mar=c(2,2,2,2),oma=c(3,3,3,5),xpd=TRUE)
	...
	lines(dd$lat,(1/ucynb_nifcell_h)*(dd$ucyn_b+1),col='red',lty=1)
	lines(dd$lat,(1/ucyna_nifcell_h)*(dd$ucyn_a+1),col='blue',lty=1)
	lines(dd$lat,(1/t_nifcell_h)*(dd$trichodesmium+1),col='dark green',lty=1)
		...
		lines(dd$lat,(1/r_nifcell_l)*(dd$richelia+1),col='orange',lty=1)
		lines(dd$lat,(1/ucynb_nifcell_l)*(dd$ucyn_b+1),col='red',lty=1)
		lines(dd$lat,(1/ucyna_nifcell_l)*(dd$ucyn_a+1),col='blue',lty=1)
		lines(dd$lat,(1/t_nifcell_l)*(dd$trichodesmium+1),col='dark green',lty=1)
		mtext(adj=0,'b)')
		
	plot(-999,ylim=c(1E-13,1E-3),log='y',xlim=c(0,20),las=1,ylab='')
	#lines(dd$lat,xxx1_l+xxx2_l+xxx3_l+xxx4_l,lwd=2)
	lines(dd$lat,(1/r_nifcell_h)*(r_Ccell_l)*(dd$richelia+1),col='orange',lty=1)
	lines(dd$lat,(1/ucynb_nifcell_h)*(ucynb_Ccell_l)*(dd$ucyn_b+1),col='red',lty=1)
	lines(dd$lat,(1/ucyna_nifcell_h)*(ucyna_Ccell_l)*(dd$ucyn_a+1),col='blue',lty=1)
	lines(dd$lat,(1/t_nifcell_h)*(t_Ccell_l)*(dd$trichodesmium+1),col='dark green',lty=1)
		mtext(side=2,expression('Carbon [mmol/m'^2*']'),line=3.5)
		mtext(adj=0,'c)')

  plot(-999,ylim=c(1E-13,1E-3),log='y',xlim=c(0,20),las=1,ylab='')
  #lines(dd$lat,xxx1_h+xxx2_h+xxx3_h+xxx4_h,lwd=2)
  lines(dd$lat,(1/r_nifcell_l)*(r_Ccell_h)*(dd$richelia+1),col='orange',lty=1)
  lines(dd$lat,(1/ucynb_nifcell_l)*(ucynb_Ccell_h)*(dd$ucyn_b+1),col='red',lty=1)
  lines(dd$lat,(1/ucyna_nifcell_l)*(ucyna_Ccell_h)*(dd$ucyn_a+1),col='blue',lty=1)
  lines(dd$lat,(1/t_nifcell_l)*(t_Ccell_h)*(dd$trichodesmium+1),col='dark green',lty=1)
